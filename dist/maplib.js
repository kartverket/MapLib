...
ISY.MapImplementation.OL3.Map = function (
  repository,
  eventHandler,
  httpHelper,
  measure,
  ...
  function _registerMapCallbacks() {
    ...
    var changeCenter = function () {
      ...
      eventHandler.TriggerEvent(
        ISY.Events.EventTypes.ChangeCenter,
        mapViewChangedObj
      );
    };
    ...
    var changeResolution = function () {
      var mapViewChangedObj = _getUrlObject();
      eventHandler.TriggerEvent(
        ISY.Events.EventTypes.ChangeResolution,
        mapViewChangedObj
      );
    };
    ...
    var mapMoveend = function () {
      ...
      var mapViewChangedObj = _getUrlObject();
      eventHandler.TriggerEvent(
        ISY.Events.EventTypes.MapMoveend,
        mapViewChangedObj
      );
    };
  ...
  function _registerMessageHandler() {
    var layerMessageHandler = new ISY.MapImplementation.OL3.Sources.CustomMessageHandler(
      eventHandler,
      _getIsySubLayerFromPool
    );
  ...
  function changeView(viewPropertyObject) {
    if (map !== undefined) {
      ...
      if (lon !== undefined && lat !== undefined) {
        var latitude =
          typeof lat === "number" ? lat : parseFloat(lat.replace(/,/g, "."));
        var longitude =
          typeof lon === "number" ? lon : parseFloat(lon.replace(/,/g, "."));
    ...
    layer.on(
      ...
      function () {
        ...
        if (layer.get("loading")) {
          ...
          eventHandler.TriggerEvent(
            ISY.Events.EventTypes.LoadingLayerEnd,
            layer
          );
        }
      },
  ...
  function _createLayer(isySubLayer) {
    ...
    var styleCallback = function (response) {
      // For caching, remember layer config
      layer.set("config", isySubLayer);
      var scales = sldstyles[isySubLayer.id].ParseSld(
        response,
        parseInt(isySubLayer.id, 10)
      );
      if (scales.maxScaleDenominator) {
        _setLayerMaxresolution(
          layer,
          _getResolutionByScale(scales.maxScaleDenominator),
          "styleCallback"
        );
      }
      if (scales.minScaleDenominator) {
        _setLayerMinresolution(
          layer,
          _getResolutionByScale(scales.minScaleDenominator),
          "styleCallback"
        );
      }
    ...
    if (layerFromPool !== null) {
      ...
      switch (isySubLayer.source) {
        case ISY.Domain.SubLayer.SOURCES.wmts:
          if (
            isySubLayer.gatekeeper &&
            isySubLayer.tiled &&
            (offline === undefined ? true : !offline.IsActive())
          ) {
            if (parameters) {
        ...
        case ISY.Domain.SubLayer.SOURCES.proxyWmts:
          ...
          break;
        case ISY.Domain.SubLayer.SOURCES.wms:
          source = new ISY.MapImplementation.OL3.Sources.Wms(
            isySubLayer,
            parameters
          );
          if (
        ...
        case ISY.Domain.SubLayer.SOURCES.wfs:
          ...
          source = new ISY.MapImplementation.OL3.Sources.Wfs(
            isySubLayer,
            offline,
            parameters
          );
        ...
        default:
          throw (
            "Unsupported source: ISY.Domain.SubLayer.SOURCES.'" +
            isySubLayer.source +
            "'. For SubLayer with url " +
            isySubLayer.url +
            " and name " +
            isySubLayer.name +
            "."
      ...
      if (isySubLayer.source === ISY.Domain.SubLayer.SOURCES.vector) {
        if (isySubLayer.style) {
          if (
            typeof isySubLayer.style === "object" ||
            isySubLayer.style.indexOf("http") < 0
          ) {
            sldstyles[
              isySubLayer.id
            ] = new ISY.MapImplementation.OL3.Styles.Json(isySubLayer.style);
            layer = new ol.layer.Vector({
              source: source,
              style: function (feature, resolution) {
                return sldstyles[isySubLayer.id].GetStyle(
                  feature,
                  _getScaleByResolution(resolution)
                );
              },
            });
          } else {
            sldstyles[
              isySubLayer.id
            ] = new ISY.MapImplementation.OL3.Styles.Sld();
            layer = new ol.layer.Vector({
              source: source,
              style: function (feature, resolution) {
                return sldstyles[isySubLayer.id].GetStyle(
      ...
      if (isySubLayer.maxScale) {
        _setLayerMinresolution(
          layer,
          _getResolutionByScale(isySubLayer.maxScale),
          "layer"
        );
      }
      if (isySubLayer.minScale) {
        _setLayerMaxresolution(
          layer,
          _getResolutionByScale(isySubLayer.minScale),
          "layer"
        );
      }
      if (returnlayer) {
  ...
  function _getLayerFromPoolByFeature(feature) {
    ...
    if (featureId === undefined) {
      ...
      if (featureId.indexOf(".") > 0) {
        ...
        var tempFeatureId =
          featureId.substr(0, featureId.indexOf("_")) +
          ":" +
          featureId.substr(featureId.indexOf("_") + 1);
      ...
      if (groups[i].isyLayers !== undefined) {
        for (var j = 0; j < groups[i].isyLayers.length; j++) {
          for (var k = 0; k < groups[i].isyLayers[j].subLayers.length; k++) {
            ...
            if (layer !== undefined) {
              layer.sortingIndex =
                groups[i].isyLayers[j].subLayers[k].sortingIndex;
            }
          }
        }
      } else {
  ...
  function getFeatureInfoUrl(isySubLayer, coordinate) {
    return (
      proxyHost +
      featureInfo.GetFeatureInfoUrl(
        isySubLayer,
        _getLayerFromPool(isySubLayer),
        coordinate,
        map.getView()
      )
    );
  ...
  function showHighlightedFeatures(layerguid, features) {
    featureInfo.ShowHighlightedFeatures(
      _getFeaturesAndAddHoverStyle(layerguid, features),
      map
    );
  }
  ...
  function _getElementsByAttribute(tag, attr, attrValue, response, exactName) {
    ...
    var elems = Array.prototype.slice.call(
      response.getElementsByTagName(tag),
      0
    );
    ...
    var matcher = function (el) {
      if (exactName) {
        ...
        return (
          el.getAttribute(attr).indexOf(attrValue) > -1 ||
          attrValue.indexOf(el.getAttribute(attr)) > -1
        );
      }
    };
  ...
  function _parseResponse(response) {
    ...
    var elementNodeByName = _getElementsByAttribute(
      "element",
      "name",
      subLayerName,
      response,
    ...
    var elementGeometryName = _getElementsByAttribute(
      "element",
      "type",
    ...
    eventHandler.TriggerEvent(ISY.Events.EventTypes.FeatureHasBeenDescribed, [
      describedSubLayer,
      describedSource,
    ]);
  }
  ...
  function getExtentForCoordinate(coordinate, pixelTolerance) {
    return featureInfo.GetExtentForCoordinate(
      coordinate,
      pixelTolerance,
  ...
  function getFeaturesInExtent(isySubLayer, extent) {
    return featureInfo.GetFeaturesInExtent(
      extent,
      _getLayerFromPool(isySubLayer),
      map.getView().getResolution()
    );
  ...
  function getHoverStyle(feature, resolution) {
    ...
    if (layer) {
      if (sldstyles[layer.guid]) {
        return sldstyles[layer.guid].GetHoverStyle(
          feature,
          _getScaleByResolution(resolution)
        );
      } else {
  ...
  */
  ...
  function transformEpsgCoordinate(coord, toCrs) {
    if (coord.epsg !== "" && toCrs !== "" && coord.epsg !== toCrs) {
      ...
      var transformedCoord = ol.proj.transform(
        [coord.lon, coord.lat],
        coord.epsg,
        toCrs
      );
  ...
  var getVectorLayers = function (isySubLayer, data) {
    var vectors = [];
    var source = ISY.MapImplementation.OL3.Sources.Vector(
      isySubLayer.subLayers[0],
  ...
  function _geolocationChange() {
    ...
    eventHandler.TriggerEvent(
      ISY.Events.EventTypes.GeolocationUpdated,
      geolocationObject
    );
  }
